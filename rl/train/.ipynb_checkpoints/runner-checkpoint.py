...
class Runner:
    ...
    def run(self):
        ...
        while total_steps <= self.c.train_steps:
            ...
            ei = self.bo.calc_int_r_and_add_points(z, next_value_ext / self.c.scale_bo_y, applied) 
            ...
            if not total_steps % self.c.test_every:
                ...
                bar.set_description(
                    f'test score {np.mean(self.test_rewards)}, steps {total_steps}, {self.c.tay_coef}')
                bar.update(self.c.test_every)
                self.test_rewards = []
                self.logger.save()

            self.trainer.agent.model.optimizer.param_groups[0]['lr'] = reduced_lr
        self.logger.save_model('NET', self.trainer.agent.model.NET)
        self.logger.save_model('AU', self.trainer.agent.model.AU)
        self.logger.save()
        self.envs.close()
